...
def check_ferry_price_from_site(vehicle_description, direction="Ro_Ge"):
    ...
    Алгоритм:
      ...
      3. Если по названию тариф не найден, пытается извлечь длину из vehicle_description.
         Если длина найдена, выбирается категория по диапазону через find_category_by_length.
      4. Если ни название, ни длина не позволяют определить категорию, то:
         - Если описание не содержит исключающих слов (например, "контейнер", "минивэн", "легков", "мото"),
           возвращается запрос на уточнение длины.
         - Иначе пытается найти тарифную категорию по совпадению ключевых слов.
      5. Если тарифная категория определена, извлекаются активное значение цены (и, если есть, зачёркнутая цена)
         для выбранного направления, а также примечание – все данные поднимаются из внешнего источника.
      6. Формируется и возвращается итоговая строка с тарифом.
    """
    # Инициализируем переменную для тарифной категории
    category = None
    ...
    if category_by_name:
        ...
        if extracted_length is not None:
            ...
            # 3. Если длина не указана, проверяем исключающие слова.
            ...
            if not any(keyword in vehicle_description.lower() for keyword in exceptions):
                ...
                # Если транспортное средство относится к типам, для которых длина не требуется,
                # пробуем найти категорию по совпадению ключевых слов.
                for key in website_prices:
...
if __name__ == "__main__":
    # Тестовый пример 1: "Фура 17 метров, Констанца-Поти, без ADR, без груза"
