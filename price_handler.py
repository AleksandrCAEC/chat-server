...
# Ключи – варианты, которые может ввести клиент (в нижнем регистре), значения – официальный термин, соответствующий тарифной категории.
TYPE_SYNONYMS = {
    "минивэн": "Minivan",
    "minivan": "Minivan",
    "фура": "Standard truck with trailer (up to 17M)",  # пример, настройте согласно данным сайта
...
def check_ferry_price_from_site(vehicle_description, direction="Ro_Ge"):
    ...
    Алгоритм:
      ...
      2. Пытается определить тарифную категорию по названию транспортного средства:
         - Если в описании встречается синоним из TYPE_SYNONYMS, используется официальный термин.
         - Иначе перебираются ключи из website_prices для поиска совпадения (без учета регистра).
      3. Если по названию не найдено, пытается извлечь длину из vehicle_description
      ...
      5. Если тарифная категория определена, извлекаются активное значение цены (и, если есть, зачёркнутая цена)
         для указанного направления, а также примечание.
    ...
    for synonym, official in TYPE_SYNONYMS.items():
        if synonym in vehicle_lower:
            if official in website_prices:
                ...
                break
    # 2. Если не найдено по синониму, пытаемся найти совпадение по ключам из website_prices
    if category is None:
    ...
    if category is None:
        ...
        if extracted_length is not None:
            ...
            # Если длина не указана, проверяем, требуется ли она для расчета
    ...
    # Извлекаем цены для выбранной категории по направлению
    if direction == "Ro_Ge":
